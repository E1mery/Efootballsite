...
/**
 * ButtonRollingText
 * A primary call-to-action whose duplicate label rolls into place on hover or keyboard focus in Motion for React.
 */
...
>(
  (
    ...
    const displayText = duplicateText ?? text;
    ...
    return (
      ...
      >
        ...
        {shouldReduceMotion ? (
          ...
          <span className="relative inline-flex overflow-hidden py-1" aria-hidden="true">
          ...
          <span className="relative inline-flex flex-col overflow-hidden py-1" aria-hidden="true">
            {/* Primary Label */}
            <motion.span
              animate={active ? { y: "-100%", opacity: 0 } : { y: "0%", opacity: 1 }}
              transition={{
        ...
        {icon && iconPosition === "right" && (
          <span className="ml-2 inline-flex shrink-0 transition-transform duration-300 group-hover/roll:translate-x-1 group-hover/roll:scale-110">
            {icon}
          </span>
        )}
      </motion.button>
