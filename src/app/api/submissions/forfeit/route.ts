...
export async function POST(req: Request) {
  try {
    ...
    if (match.matchDate && now < new Date(match.matchDate) && !isReopenedByAdmin) {
      ...
      return NextResponse.json(
        {
          error: `Fixture Not Dropped Yet: This match is scheduled to drop on ${dropDateFormatted} at 12:00 AM (Midnight). Forfeit claims unlock once the fixture officially drops.`,
        },
        { status: 400 }
      );
    }
